#!/usr/bin/env python3
"""
Generate tweet previews and store them locally for testing.
"""
...
def create_tweet_preview(analysis_data, include_chart=True):
...
def save_tweet_preview(tweet_data, analysis_file):
    ...
    # Save JSON data
    json_file = previews_dir / f"{analysis_filename}_tweet_{timestamp}.json"
    with open(json_file, 'w', encoding='utf-8') as f:
        json.dump(tweet_data, f, indent=2, ensure_ascii=False)
    
    # Create HTML preview
    html_file = previews_dir / f"{analysis_filename}_preview_{timestamp}.html"
    create_html_preview(tweet_data, html_file)
    
    # Create markdown preview
    md_file = previews_dir / f"{analysis_filename}_preview_{timestamp}.md"
    create_markdown_preview(tweet_data, md_file)
    
    return json_file, html_file, md_file

def create_html_preview(tweet_data, output_file):
...
def create_markdown_preview(tweet_data, output_file):
...
def generate_preview_from_latest():
    ...
    print(f"📊 Generating tweet preview from: {latest_file.name}")
    ...
    tweet_data = create_tweet_preview(analysis_data)
    
    # Save preview files
    json_file, html_file, md_file = save_tweet_preview(tweet_data, latest_file)
    
    print(f"✅ Tweet preview generated:")
    ...
    if tweet_data['character_count'] > 280:
        print("⚠️  WARNING: Tweet is too long!")
    else:
...
def main():
    """Main function."""
    print("🐦 Tweet Preview Generator")
    print("=" * 40)
    
    if generate_preview_from_latest():
        print(f"\n🎉 Preview generated successfully!")
        print(f"📁 Check the 'tweet_previews' folder for all files")
    else:
